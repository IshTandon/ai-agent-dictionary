...
function buildSession(allTerms: Term[]): SessionItem[] {
  const p = getProgress();
  const viewed = new Set(p.viewed_terms);
  const lastSeen = p.last_seen || {};
  const rand = seedRandom(todayISO());
...
export default function SessionPage() {
  ...
  if (alreadyDone) {
    return <SessionDone xpEarned={0} newCount={0} reviewCount={0} streakOld={0} streakNew={0} alreadyCompleted />;
  }
  ...
  return <SessionFlow allTerms={allTerms} />;
}

function SessionFlow({ allTerms }: { allTerms: Term[] }) {
  const router = useRouter();
  const session = useMemo(() => buildSession(allTerms), [allTerms]);
  ...
  return (
    <div className="mx-auto flex min-h-[calc(100vh-60px)] max-w-lg flex-col px-5 py-6">
      ...
      <div className="flex flex-1 flex-col">
        {current.kind === 'new' ? (
          <NewTermStep item={current} onXP={handleXP} onAdvance={advance} />
        ) : (
          <ReviewStep item={current} onXP={handleXP} onAdvance={advance} allTerms={allTerms} />
        )}
      </div>
    </div>
  );
...
function ReviewStep({
  ...
  if (hasQuiz) {
    return <ReviewQuiz term={term} onXP={onXP} onAdvance={onAdvance} />;
  }

  return <ReviewFlipCard term={term} onXP={onXP} onAdvance={onAdvance} />;
}
  ...
  return (
    <div className={`flex flex-1 flex-col animate-fade-in ${shaking ? 'shake' : ''}`}>
        ...
        {quiz.options.map((option, i) => {
          ...
          return (
            <button
              ...
              style={{ backgroundColor: bg, border, cursor: revealed ? 'default' : 'pointer' }}
            >
      ...
      {revealed && (
        ...
        >
          <p className="text-[12px] font-bold" style={{ color: selected === correctIndex ? '#10B981' : '#EF4444' }}>
            {selected === correctIndex ? `Correct! +25 XP` : 'Not quite — no XP this time'}
          </p>
  ...
  if (alreadyCompleted) {
    ...
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-md flex-col items-center justify-center px-5 text-center">
        ...
        <p className="mb-8 text-[14px]" style={{ color: 'var(--color-muted)' }}>
          Come back {tomorrowStr} for your next session.
        </p>
        <Link
          href="/"
          className="rounded-xl px-6 py-3 text-[14px] font-medium"
  ...
  return (
    <div className="mx-auto flex min-h-[60vh] max-w-md flex-col items-center justify-center px-5 text-center animate-fade-in">
      ...
      <div className="flex w-full gap-3">
        <Link
          href="/"
          className="flex-1 rounded-xl py-3 text-center text-[14px] font-medium"
